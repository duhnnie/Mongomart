...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        this.db.collection("item").aggregate(pipeline).toArray(function (err, categories) {
            categories.push({
                _id: "All",
                num: categories.reduce((prev, current) => { return prev + current.num }, 0)
            });
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
         ...
         */
         ...
         cursor = this.db.collection("item").find(query);
         cursor.sort({_id: 1});
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        var cursor = this.db.collection("item").find(query).count((err, count) => {
            callback(count);
        });
        /*
        * TODO-lab2B
